<template>
  <view class="app-shell">
    <StatusBar />
    <view class="header">
      <text class="header-title">片库</text>
...
export default {
  ...
  methods: {
    ...
    _restoreFromStorage() {
      try {
        const raw = uni.getStorageSync('vmk_library_cache')
        if (!raw) return false
    ...
    _saveToStorage() {
      try {
        uni.setStorageSync('vmk_library_cache', {
          list: this.list,
          cats: this.cats,
          cat: this.cat,
    ...
    async loadList(reset = false, silent = false) {
      ...
      const catName = this.cats[this.cat] || '全部'
      const targetPg = this.page
      console.log('[library] loadList: cat=' + catName + ', pg=' + targetPg + ', reset=' + reset + ', silent=' + silent)
      const res = await fetchLibraryListPaged(catName, targetPg)
      if (res && Array.isArray(res.list)) {
        if (res.list.length) {
          ...
          console.log('[library] loadList ok: count=' + res.list.length + ', hasMore=' + this.hasMore + ', nextPg=' + this.page)
          // 首屏数据就绪后持久化，下次冷启动秒开
        ...
        console.warn('[library] loadList FAIL: 请求失败，保留状态，下次重试 pg=' + targetPg)
      }
    ...
    async selectCategory(i) {
      ...
      console.log('[library] selectCategory: ' + this.cats[i])
      this.cat = i
      this.loading = true
      await this.loadList(true)
    },
    ...
    onScrollToLower() {
      console.log('[library] ★ scrolltolower! hasMore=' + this.hasMore + ', listLoading=' + this.listLoading)
      if (this.listLoading || !this.hasMore || !this.initialized) return
      this.loadList(false)
    },
    /**
     * 首屏内容不足时自动补载，确保出现滚动条（参考首页实现）
     * 安全锁：最多自动补载 3 次
     */
    _ensureScrollable() {
      ...
      if (!sv) sv = this.$refs.libScrollView && this.$refs.libScrollView.$el ? this.$refs.libScrollView.$el : null
      if (!sv) sv = this.$el.querySelector('.page-content')
      if (!sv) return
      const sh = sv.scrollHeight || 0
      const ch = sv.clientHeight || 0
      const canScroll = sh > ch + 10
      console.log('[library] ensureScrollable: autoLoad=' + this._autoLoadCount + '/3, sh=' + sh + ', ch=' + ch + ', canScroll=' + canScroll + ', hasMore=' + this.hasMore + ', len=' + this.list.length)
